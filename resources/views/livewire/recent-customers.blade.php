<div class="relative grid p-5 bg-white recent-customers rounded-3xl">
    <div class="flex items-start justify-between card-header">
        <h2 class="text-2xl font-bold text-red-500">Recent Customers</h2>
        <a class="relative px-3 py-1 text-white bg-red-500 rounded-md button" href="/customers">View All</a>
    </div>
    <table class="w-full mt-3 border-collapse">
        ...
        <tbody wire:poll>
            @php
            $top11 = $customers->sortByDesc('created_at')->take(11);
            @endphp

            @foreach ($top11 as $customer)
            <tr class="text-gray-500 border-b border-gray-400 border-opacity-20 hover:bg-red-500 hover:text-white {{$loop->last ? 'border-none' : ''}}">
                ...
                <td class="p-3 text-right">
                    <span class="actions">
                        <button class="px-1 pt-1 text-lg text-white bg-yellow-500 rounded-md leading-0" wire:click='$emit("openModal", "edit-customer", {{ json_encode(["customerId" => $customer->id]) }})'>
